...
NS.IS_MOBILE="function"==typeof window.matchMedia&&window.matchMedia("(max-width: 768px)").matches,
...
},CONFIG_DEFAULTS={showIgnScore:!0,showUserRating:!0,
showReviewGrading:!0,showReview:!0,showSteamReviews:!0,
showAward:!0,showEsrb:!0,showDeveloper:!1,showPublisher:!1,
showGenres:!0,showPlatforms:!0,showFeatures:!1,
showDescription:!0,showHltb:!0,showLeisure:!0,
showHltbSearchFallback:!0};function currentPlatform(){
...
leisure:["showLeisure"]
},NS.DEFAULT_SECTION_ORDER=DEFAULT_SECTION_ORDER,NS.getSectionOrderFor=function(platform){
...
NS.getSettingsShared=()=>NS.storage.getSync("placementShared",!1),
...
;return NS.getSettingsShared()?enabled.slice(0,1):enabled
...
const platform=currentPlatform(),newValue=!NS.getConfigFor(key,platform)
;(NS.getSettingsShared()?NS.PLATFORMS:[platform]).forEach(p=>NS.setConfigFor(key,p,newValue)),
NS.registerMenuCommands()}
}(window.IGN_METADATA_INJECTOR=window.IGN_METADATA_INJECTOR||{}),
function(NS){"use strict";const ESRB_FULL_NAMES={
e:"Everyone",everyone:"Everyone","e10+":"Everyone 10+",
...
;if(row)return{element:row,position:"before"}}
if("belowLeftSidebar"===pref){const anchor=function(){
...
;return fallbackRow?{element:fallbackRow,position:"after",
alignTo:document.querySelector("aside")&&document.querySelector("aside").previousElementSibling||fallbackRow
}:null}();if(anchor)return anchor}
...
;return displayData.length>0?hltbSectionHtml("HowLongToBeat","#66c0f4",displayData,hltbUrl):NS.getConfig("showHltbSearchFallback")?function(hltbUrl){
if(!hltbUrl)return""
;const label=/\/game\//i.test(hltbUrl)?"View on HowLongToBeat":"Search on HowLongToBeat"
...
const opts=NS.LOCATION_OPTIONS.map(opt=>`<option value="${opt.value}" ${"belowGameMedia"===opt.value?"selected":""}>${NS.escapeHtml(opt.label)}</option>`).join(""),heading=NS.SECTION_LABELS[key]||key,select=`<div><select id="ign_${key}_location_${getEffectivePlatform()}" class="ign_settings_select" data-key="${key}">${opts}</select></div>`
...
NS.setSettingsShared(e.target.checked),
NS.openSettingsPanel()})
;const pagerPrev=overlay.querySelector("#ign_pager_prev"),pagerNext=overlay.querySelector("#ign_pager_next")
;function flipPager(direction){
const enabled=NS.getEnabledPlatforms(),idx=enabled.indexOf(getEffectivePlatform())
...
sectionKey in s.visibleMap&&(NS.SECTION_CONFIG_KEYS[sectionKey]||[]).forEach(configKey=>NS.setConfigFor(configKey,p,s.visibleMap[sectionKey]))
}),
NS.setSectionOrderFor(p,s.order),null!==s.combineAll&&void 0!==s.combineAll&&NS.setCombineAllFor(p,s.combineAll),
null!==s.hltbSearchFallback&&void 0!==s.hltbSearchFallback&&NS.setConfigFor("showHltbSearchFallback",p,s.hltbSearchFallback),
void 0!==s.positionValue&&NS.setBadgePositionFor(p,s.positionValue),
s.order.forEach(key=>{
NS.setSectionLocationFor(key,p,s.locationMap.hasOwnProperty(key)?s.locationMap[key]:"inline")
})}
pagerPrev&&pagerPrev.addEventListener("click",()=>flipPager(-1)),pagerNext&&pagerNext.addEventListener("click",()=>flipPager(1)),
...
const shared=NS.getSettingsShared(),platform=getEffectivePlatform()
;if(shared){const visibleMap={}
;list.querySelectorAll(".ign_visible_checkbox").forEach(cb=>{
visibleMap[cb.dataset.key]=cb.checked})
;const order=Array.from(list.querySelectorAll(".ign_order_item")).map(li=>li.dataset.key),combineAll=!!combineAllCheckbox&&combineAllCheckbox.checked,hltbSearchFallbackCb=overlay.querySelector("#ign_hltb_search_fallback"),hltbSearchFallback=!!hltbSearchFallbackCb&&hltbSearchFallbackCb.checked,posSel=overlay.querySelector(`#ign_badge_position_${platform}`),locationMap={}
;order.forEach(key=>{
const sel=overlay.querySelector(`#ign_${key}_location_${platform}`)
;locationMap[key]=sel?sel.value:"inline"});const s={
order:order,visibleMap:visibleMap,combineAll:combineAll,
hltbSearchFallback:hltbSearchFallback,
locationMap:locationMap,
positionValue:posSel?posSel.value:void 0}
;NS.PLATFORMS.forEach(p=>writeSettingsForPlatform(p,s))
}else{const visibleMap={}
;list.querySelectorAll(".ign_visible_checkbox").forEach(cb=>{
visibleMap[cb.dataset.key]=cb.checked})
;const order=Array.from(list.querySelectorAll(".ign_order_item")).map(li=>li.dataset.key),combineAll=!!combineAllCheckbox&&combineAllCheckbox.checked,hltbSearchFallbackCb=overlay.querySelector("#ign_hltb_search_fallback"),hltbSearchFallback=!!hltbSearchFallbackCb&&hltbSearchFallbackCb.checked,posSel=overlay.querySelector(`#ign_badge_position_${platform}`),locationMap={}
;order.forEach(key=>{
const sel=overlay.querySelector(`#ign_${key}_location_${platform}`)
;locationMap[key]=sel?sel.value:"inline"
}),writeSettingsForPlatform(platform,{order:order,
visibleMap:visibleMap,combineAll:combineAll,
hltbSearchFallback:hltbSearchFallback,
locationMap:locationMap,
positionValue:posSel?posSel.value:void 0
}),Object.keys(platformDrafts).forEach(otherPlatform=>{
if(otherPlatform===platform)return
;const snap=platformDrafts[otherPlatform];if(!snap)return
;const locMap={};snap.order.forEach(key=>{
...
}),writeSettingsForPlatform(otherPlatform,{order:snap.order,
visibleMap:snap.visible,combineAll:snap.combineAll,
hltbSearchFallback:snap.hltbSearchFallback,
locationMap:locMap,
positionValue:snap.positionSelects[`ign_badge_position_${otherPlatform}`]
})})}
overlay.remove(),platformDrafts={},NS.registerMenuCommands(),refreshBadgeNow()
...
;let score=function(targetTokens,candidateTokens,storeYear,candidateObj){
...
;score>=0&&EDITION_QUALIFIER_TEST_RE.test(name)&&(score-=.35),
...
return hltbUrl&&!/\/game\//i.test(hltbUrl)&&(hltbUrl=""),{
fetchedGameTitle:fetchedGameTitle,ignScore:ignScore,
...
},lookupKey=gameTitle.toLowerCase().trim(),userOverride=NS.getUserOverrideForTitle(gameTitle),directHltbUrl=userOverride&&userOverride.hltbUrl||NS.HLTB_DIRECT_URL_OVERRIDES[lookupKey],overrideUrl=NS.IGN_SOURCE_OVERRIDES[lookupKey]
